...
import { ExtraSlotPaymentForm } from "@/components/extra-slot-payment-form";
import { decodeSessionToken } from "@/lib/session";
import { prisma } from "@/lib/prisma";

export default async function ExtraSlotPaymentPage() {
  const cookieStore = await cookies();
  const sessionToken = cookieStore.get("zd_session")?.value;
  const userId = decodeSessionToken(sessionToken);

  if (!userId) {
    redirect("/login?next=/payment/extra-slot");
  }
  ...
  if (!user.teamId) {
    redirect("/teams?next=/payment/extra-slot");
  }

  if (user.team?.extraSlotUnlocked) {
    redirect("/teams?extraSlot=unlocked");
  }
      ...
      <div className="relative z-10 mx-auto w-full max-w-3xl px-4 pt-6 sm:px-6 lg:px-8">
        <header className="rounded-xl border border-white/10 bg-black/40 px-4 py-3 backdrop-blur-md mb-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm font-semibold tracking-[0.2em] text-phosphor">ZERO_DAY // EXTRA SLOT PAYMENT</p>
