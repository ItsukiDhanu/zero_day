...
type JoinLinkFlowProps = {
  link: string;
};

function getRegistrationUrl(link: string) {
  return `/register?next=${encodeURIComponent(`/teams/join-link/${link}`)}`;
}

export function JoinLinkFlow({ link }: JoinLinkFlowProps) {
    ...
    const handleJoin = async () => {
      try {
        ...
        const response = await fetch("/api/teams/join-link", {
          ...
          body: JSON.stringify({ joinLink: link }),
        });
        ...
        if (!response.ok) {
          if (response.status === 401) {
            router.replace(getRegistrationUrl(link));
            return;
          }
